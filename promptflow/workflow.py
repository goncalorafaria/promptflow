...
from promptflow.process import Process

from promptflow.functools import snd
...
class WorkFlow:
    ...
    async def __forward_and_run(self, *args, **kwargs) -> List[Any]:
        ...
        for result in results:
            ...
            no_meta_results.append(list(map(snd, result)))

        if len(no_meta_results) == 1:
            return no_meta_results[0]
        else:
            return no_meta_results
    ...
    def __or__(self, other):
        if isinstance(other, WorkFlow):
            ...
            return Sequential(self, UnitaryWorkflow(other))
        else:
            raise ValueError("Invalid operand type.")


### this will allow for syntatic sugar.
class Sequential(WorkFlow):
    """Workflow generated by the sequenctial composition of workflows."""

    def __init__(self, *flows: List[WorkFlow]):
    ...
    def forward(self, *args, **kwargs) -> Actor:
...
class UnitaryWorkflow(WorkFlow):
